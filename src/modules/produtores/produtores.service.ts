...
export class ProdutoresService {
  ...
  async create(createProdutorDto: CreateProdutorDto): Promise<Produtor> {
    await this.validateAreas(createProdutorDto);
  ...
  async update(
    ...
    await this.validateAreas(updateProdutorDto);
  ...
  private async validateAreas(dto: CreateProdutorDto): Promise<void> {
    const { area_total_hectares, area_agricultavel_hectares, area_vegetacao_hectares } = dto;
